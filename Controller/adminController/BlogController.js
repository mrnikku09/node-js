...
const addblogprocess = async (req, res) => {
    ...
    var filename =req.file? ENV.APP_URL + ENV.SITE_UPLOAD_PATH + req.file.filename:''
    ...
    if (_id && _id != "") {
        // return res.send('yes')
        var blogObj = await blogModel.findOneAndUpdate({ _id: _id }, { blog_name, blog_desc, blog_image: filename }, { new: true })

    } else {
        // return res.send('no')
        var blogObj = await blogModel.create({ blog_name, blog_desc, blog_user_id: userId._id,blog_image: filename })
